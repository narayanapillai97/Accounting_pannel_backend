...
exports.addVendor = async (req, res) => {
  ...
  try {
    ...
    if (!vendor_name || !mobile_number || !email) {
      return res.status(400).json({ success: false, message: "vendor_name, mobile_number and email are required." });
    }
    ...
    vendorMaster.create(
      ...
      (err, result) => {
        if (err) {
          ...
          return res.status(500).json({ success: false, message: err.sqlMessage || err.message || "Error while adding vendor." });
        }
        res.status(201).json({ success: true, message: "Vendor added successfully!", vendorId: result.insertId });
      }
    );
  } catch (error) {
  ...
  try {
    ...
    vendorMaster.getAll((err, results) => {
      if (err) {
        ...
        return res.status(500).json({ error: err.sqlMessage || err.message || "Error retrieving vendors." });
      }
  ...
  try {
    ...
    vendorMaster.getById(id, (err, result) => {
      if (err) {
        ...
        return res.status(500).json({ error: err.sqlMessage || err.message || "Error retrieving vendor." });
      }
...
exports.updateVendorById = async (req, res) => {
  ...
  try {
    ...
    if (!vendor_name || !mobile_number || !email) {
      return res.status(400).json({ error: "vendor_name, mobile_number and email are required." });
    }
    ...
    vendorMaster.updateById(
      ...
      (err, result) => {
        if (err) {
          ...
          return res.status(500).json({ error: err.sqlMessage || err.message || "Error updating vendor." });
        }
  ...
  try {
    ...
    vendorMaster.deleteById(id, (err, result) => {
      if (err) {
        ...
        return res.status(500).json({ error: err.sqlMessage || err.message || "Error deleting vendor." });
      }
      if (result.affectedRows === 0) return res.status(404).json({ error: "Vendor not found." });
